...
$all_data=[];
$data=$con->prepare("SELECT * FROM accounts WHERE deleted=1 LIMIT $startFrom, $recordsPerPage");
$data->execute();
...
// Pagination links
$sql = "SELECT COUNT(*) AS total FROM accounts WHERE deleted=1";
          ...
          <div class='modal-dialog modal-dialog-centered'>
            <div class='modal-content'>
              <div class='modal-header bg-dark my-0'>
                <h1 class='modal-title fs-5 text-white' id='modal-label'>Seguro que quieres borrar <p class="text-danger d-inline">permanentemente</p> la cuenta <span class='text-success'><?=$account_name?></span>? <p class="text-danger d-inline">(esta accion no es reversible)</p></h1>
              </div>
              <div class='modal-body p-0'>
                <div class='container-fluid my-0 justify-content-center form-signin'>
                  <form id='delete-account-<?=$id?>' class='row g-3' role='form' name='delete-account-<?=$id?>' action='php/actions/delete_account_def.php?id=<?=$id?>' method='post'></form>
                </div>
              </div>
